...
<script type="text/javascript">
	...
	function update(pageNumber, qna_number){
		alert(1);
		window.open('update.qna?pageNumber=' + pageNumber + "&qna_number=" + qna_number, 'QnA 수정', 'width=430, height=500, margin=auto, scrollbars=yes');
	}
</script>
				...
				<tr>
					<td colspan="6">
						...
						<c:if test="${ qnaBean.member_id == member_id }">
							<input type="button" value="수정" style="float: right;" onClick="update('${ pageInfo.pageNumber }', '${ qnaBean.qna_number }')">
							<input type="button" value="삭제" style="float: right;" onClick="location.href='delete.qna?pageNumber=${ pageInfo.pageNumber }&qna_number=${ qnaBean.qna_number }'">
						</c:if>
						<c:if test="${ member_id == 'admin' }">
									<input type="button" value="답변작성(관리자)" style="float: right;" onClick="location.href='reply.qna?ref=${ qnaBean.ref }&pageNumber=${ pageNumber }&qna_number=${ qnaBean.qna_number }'">
						</c:if>
					</td>
				</tr>
			</c:if>
